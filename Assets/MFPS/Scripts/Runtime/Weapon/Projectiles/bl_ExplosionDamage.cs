...
public class bl_ExplosionDamage : bl_ExplosionBase
{
    ...
    {
        if (explosionType == ExplosionType.Shake && !bl_GameData.CoreSettings.ArriveKitsCauseDamage)
            return;
    ...
    void DoCollisionDamage()
    {
        ...
        foreach (Collider c in colls)
        {
            ...
            if (!c.transform.TryGetComponent<IMFPSDamageable>(out var damageable))
            {
                ...
                {
                    if (!c.attachedRigidbody.TryGetComponent<IMFPSDamageable>(out damageable)) continue;
                }
                else continue;
            }
    ...
    private bool IsMyInRange()
    {
        ...
        return bl_UtilityHelper.Distance(transform.position, p.transform.position) <= this.explosionRadius;
    }
