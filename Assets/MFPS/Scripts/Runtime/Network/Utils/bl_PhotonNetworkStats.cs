...
public class bl_PhotonNetworkStats : MonoBehaviour, IConnectionCallbacks
{
    ...
    [Header("GUI")]
    [SerializeField] Rect m_GUIArea = new(25, 10, 300, 500);
    ...
    public List<StatInfo> ValuesList = new();
    public List<StatInfo> LastValuesList = new();
    ...
    IEnumerator Start()
    {
        ...
        if (!bl_GameData.CoreSettings.ShowNetworkStats) { Destroy(gameObject); yield break; }

        PhotonNetwork.AddCallbackTarget(this);
        bl_PhotonNetworkStats pns = FindAnyObjectByType<bl_PhotonNetworkStats>();
    ...
    void GetValues()
    {
        ...
        {
            Variable = "Out Calls",
            Value = gls.TotalOutgoingMessageCount.ToString()
        };
        ValuesList.Add(stat);
        ...
        {
            Variable = "In Calls",
            Value = gls.TotalIncomingMessageCount.ToString()
        };
        ValuesList.Add(stat);
    ...
    private void OnGUI()
    {
        if (!isInitialized || !bl_PhotonNetwork.IsConnected) return;
    ...
    public void OnConnected()
    {
        if (LogTrafficStats)
        {
            ...
            this.InvokeRepeating(nameof(LogStats), UpdateRate, UpdateRate);
        }
    }
