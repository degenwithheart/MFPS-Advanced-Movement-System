...
public class bl_LobbyChat : MonoBehaviour, IChatClientListener
{
    ...
    IEnumerator Start()
    {
        ...
        if (!bl_GameData.CoreSettings.UseLobbyChat)
        {
            ChatRoot.SetActive(false);
            this.enabled = false;
            yield break;
...
#endif
        this.chatClient.Connect(PhotonNetwork.PhotonServerSettings.AppSettings.AppIdChat, version, new Photon.Chat.AuthenticationValues(bl_PhotonNetwork.NickName));
    }
    ...
    public void OnConnected()
    {
        ...
        UserIdText.text = "Connected as " + bl_PhotonNetwork.NickName;
        this.ChatPanel.gameObject.SetActive(true);
        this.chatClient.SetOnlineStatus(ChatUserStatus.Online); // You can set your online state (without a mesage).
        ChannelText.text = selectedChannelName.ToUpper();
        ChatButton.SetActive(bl_GameData.CoreSettings.UseLobbyChat);
    ...
    public static bl_LobbyChat Instance
    {
        ...
        {
            if (_instance == null)
                _instance = FindAnyObjectByType<bl_LobbyChat>();
            return _instance;
        }
    }
}