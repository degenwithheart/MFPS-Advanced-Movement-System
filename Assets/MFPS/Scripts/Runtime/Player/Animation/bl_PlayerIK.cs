...
public class bl_PlayerIK : bl_PlayerIKBase
{
    ...
    void OnIKMotionWeightModified(bool enter, Animator sourceAnim, bl_AnimatorIKBlendEvent.Modifier modifier)
    {
        ...
        float smoothTime = enter ? modifier.SmoothTime : modifier.OutSmoothTime;
    ...
    void LegsIK()
    {
        // Adjustments for smoother transitions
        ...
        // Pelvis adjustment variables
        float leftFootOffset = 0f;
        float rightFootOffset = 0f;
        Vector3 pelvisPosition = CachedTransform.position;

        // Left foot placement
        if (Physics.Raycast(leftFootPos + upStart, Vector3.down, out footRaycast, detectionRange, FootLayers))
        {
            leftFootPos.y = footRaycast.point.y + TerrainOffset;

            // Get the surface normal from raycast
            Vector3 leftFootNormal = footRaycast.normal;

            // Calculate the foot rotation relative to the surface normal
            Quaternion leftFootRotation = Quaternion.LookRotation(Vector3.ProjectOnPlane(CachedTransform.forward, leftFootNormal), leftFootNormal);

            // Apply left foot IK
            animator.SetIKPosition(AvatarIKGoal.LeftFoot, leftFootPos);
            animator.SetIKRotation(AvatarIKGoal.LeftFoot, Quaternion.Slerp(animator.GetIKRotation(AvatarIKGoal.LeftFoot), leftFootRotation, deltaTime * 8f));  // Smoother rotation blending

            // Adjust knee position
            animator.SetIKHintPosition(AvatarIKHint.LeftKnee, CachedTransform.TransformPoint(leftKneeTarget));

            // Smooth foot IK weight transition
            lFootIKWeight = Mathf.Lerp(lFootIKWeight, 1, deltaTime * 10f);

            // Store the height offset for pelvis adjustment
            leftFootOffset = footRaycast.point.y - pelvisPosition.y;
        }
        else
        {
            lFootIKWeight = Mathf.Lerp(lFootIKWeight, 0, deltaTime * 4f);
        }

        // Right foot placement
        if (Physics.Raycast(rightFootPos + upStart, Vector3.down, out footRaycast, detectionRange, FootLayers))
        {
            rightFootPos.y = footRaycast.point.y + TerrainOffset;

            // Get the surface normal from raycast
            Vector3 rightFootNormal = footRaycast.normal;

            // Calculate the foot rotation relative to the surface normal
            Quaternion rightFootRotation = Quaternion.LookRotation(Vector3.ProjectOnPlane(CachedTransform.forward, rightFootNormal), rightFootNormal);

            // Apply right foot IK
            animator.SetIKPosition(AvatarIKGoal.RightFoot, rightFootPos);
            animator.SetIKRotation(AvatarIKGoal.RightFoot, Quaternion.Slerp(animator.GetIKRotation(AvatarIKGoal.RightFoot), rightFootRotation, deltaTime * 8f));  // Smoother rotation blending

            // Adjust knee position
            animator.SetIKHintPosition(AvatarIKHint.RightKnee, CachedTransform.TransformPoint(rightKneeTarget));

            // Smooth foot IK weight transition
            rFootIKWeight = Mathf.Lerp(rFootIKWeight, 1, deltaTime * 10f);

            // Store the height offset for pelvis adjustment
            rightFootOffset = footRaycast.point.y - pelvisPosition.y;
        }
        else
        {
            rFootIKWeight = Mathf.Lerp(rFootIKWeight, 0, deltaTime * 4f);
        }

        // Pelvis adjustment for better foot-ground alignment
        float averageFootOffset = (leftFootOffset + rightFootOffset) / 2f;
        pelvisPosition.y += Mathf.Lerp(0f, averageFootOffset, deltaTime * 5f);
        CachedTransform.position = pelvisPosition;

        // Handle rotation weight adjustment during turning
        if (turnValue > 0.1f)
        {
            lFootIKWeight = Mathf.Lerp(lFootIKWeight, 0, deltaTime * 6f);
            rFootIKWeight = Mathf.Lerp(rFootIKWeight, 0, deltaTime * 6f);
        }

        // Set final IK weights for feet and knees
