...
public class bl_PlayerAnimations : bl_PlayerAnimationsBase
{
    ...
    void ControllerInfo()
    {
        ...
        // if rotation less than 5 degrees per second, consider it's not rotating
        if (rotationSpeed < deltaTime * 5)
        {
            turnAngle = 0;
            smoothedTurnAngle = 0;
        }
        ...
        smoothedTurnAngle = Mathf.MoveTowards(smoothedTurnAngle, turnAngle, deltaTime);
    ...
    public override void PlayFireAnimation(GunType typ, FireReplicationFlag flag = FireReplicationFlag.None)
    {
        ...
        switch (typ)
        {
            ...
            case GunType.Machinegun:
                if (flag.IsByteEnumFlagPresent(FireReplicationFlag.BashAttack)) Animator.Play("RifleMeleeAttack", 1, 0);
                else Animator.Play("RifleFire", 1, 0);
                break;
            case GunType.Pistol:
                if (flag.IsByteEnumFlagPresent(FireReplicationFlag.BashAttack)) Animator.Play("PistolMeleeAttack", 1, 0);
                else Animator.Play("PistolFire", 1, 0);
                break;
            case GunType.Launcher:
                Animator.Play("LauncherFire", 1, 0);
                break;
            case GunType.Shotgun:
                if (flag.IsByteEnumFlagPresent(FireReplicationFlag.BashAttack)) Animator.Play("ShotgunMeleeAttack", 1, 0);
                else Animator.Play("ShotgunFire", 1, 0);
                break;
        }
    }
