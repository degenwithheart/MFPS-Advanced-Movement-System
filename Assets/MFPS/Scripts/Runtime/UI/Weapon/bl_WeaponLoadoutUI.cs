...
public class bl_WeaponLoadoutUI : bl_WeaponLoadoutUIBase
{
    ...
    public override void SetLoadout(List<bl_Gun> guns)
    {
        for (int i = 0; i < SlotsGroups.Length; i++)
        {
            if (IconsImg != null) IconsImg[i].canvasRenderer.SetColor(Color.white);
        ...
        current = 0;
        if (IconsImg != null) IconsImg[0].canvasRenderer.SetColor(Color.black);
    }
    ...
    {
        if (IconsImg == null || newGun == null) { return; }
    ...
    /// 
    /// </summary>
    public override void SetActiveSlot(int nextSlot)
    {
        if (!bl_GameData.CoreSettings.ShowWeaponLoadout || !gameObject.activeSelf) return;
        ...
        if (changeCoroutine != null)
        {
            ...
            {
                if (IconsImg != null && IconsImg[i] != null)
                    IconsImg[i].canvasRenderer.SetColor(Color.white);
            }
        }
        changeCoroutine = StartCoroutine(ChangeSlot(nextSlot));
    }
    ...
    {
        content.SetActive(layers.IsEnumFlagPresent(RoomUILayers.Loadout));
    }
}