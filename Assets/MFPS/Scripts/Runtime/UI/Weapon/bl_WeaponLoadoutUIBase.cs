...
public abstract class bl_WeaponLoadoutUIBase : MonoBehaviour
{
    ...
    public abstract void SetActiveSlot(int slotID);
    ...
    public static bl_WeaponLoadoutUIBase Instance
    {
        get
        {
            if (_instance == null) { _instance = FindAnyObjectByType<bl_WeaponLoadoutUIBase>(); }
            if (_instance == null && bl_UIReferences.Instance != null) { _instance = bl_UIReferences.Instance.GetComponentInChildren<bl_WeaponLoadoutUIBase>(true); }
            return _instance;
        }
    }
}
