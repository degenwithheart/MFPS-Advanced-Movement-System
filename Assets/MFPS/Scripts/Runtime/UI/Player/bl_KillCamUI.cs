...
public class bl_KillCamUI : bl_KillCamUIBase
{
    ...
    public override void Show(bl_KillCamBase.KillCamInfo kcinfo)
    {
        ...
        if (kcinfo.IsTargetChangeOnly) return;
        ...
        if (bl_MFPS.RoomGameMode.CurrentGameModeData.onPlayerDie == GameModeSettings.OnPlayerDie.SpawnAfterRoundFinish)
        {
    ...
    {
        float d = 0;
        float rt = bl_GameData.CoreSettings.PlayerRespawnTime;
        while (d < 1)
        {
            d += Time.deltaTime / rt;
