...
namespace MFPS.Runtime.UI.Bindings
{
    public class bl_KillFeedUIBinding : bl_KillFeedUIBindingBase
    {
        ...
        [SerializeField] private GameObject localIndicator = null;
        ...
        void OnKillMessage(KillFeed info)
        {
            ...
            KillerText.color = info.KillerTeam.GetTeamColor();
            KilledText.color = GetOppositeTeam(info.KillerTeam).GetTeamColor();
            if (localIndicator != null) localIndicator.SetActive(isLocalPlayerName(info.Killed) || isLocalPlayerName(info.Killer));
            if (bl_GameData.CoreSettings.killFeedWeaponShowMode == KillFeedWeaponShowMode.WeaponName)
            {
