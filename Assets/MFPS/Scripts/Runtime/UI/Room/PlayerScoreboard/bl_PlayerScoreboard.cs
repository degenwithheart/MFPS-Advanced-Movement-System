...
public class bl_PlayerScoreboard : bl_PlayerScoreboardBase
{
    ...
    readonly Dictionary<int, bl_PlayerScoreboardUIBase> cachedUIBindings = new();
    readonly Dictionary<MFPSBotProperties, bl_PlayerScoreboardUIBase> cachedBotsUIBindings = new();
    bool botsScoreInstance = false;
    private List<bl_PlayerScoreboardUIBase> cachePlayerScoreboardSorted = new();
    private List<bl_PlayerScoreboardUIBase> cachePlayerScoreboardSorted2 = new();
    #endregion
    ...
    public void UpdateTables()
    {
        ...
        for (int i = 0; i < players.Length; i++)
        {
            ...
            if (p.GetPlayerTeam() != Team.None)
            {
                ...
                {
                    bl_PlayerScoreboardUIBase script;
                    if (!isOneTeamMode)
                    {
    ...
    void SortScoreboard()
    {
        ...
        else
        {
            ...
            var all = new List<bl_PlayerScoreboardUIBase>();
    ...
    {
        SetActive(active, (bl_GameManager.Instance != null && !bl_GameManager.Instance.FirstSpawnDone && bl_GameManager.Joined));
    }
    ...
    {
        Debug.Log("Removed Binding: " + uiBinding.name);
