...
public class bl_KickVotationUI : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        KickManager = FindAnyObjectByType<bl_KickVotation>();
    }
    ...
    {
        // if the local player is the master client, then he can kick the player directly
        if (bl_PhotonNetwork.IsMasterClient && bl_GameData.CoreSettings.MasterCanKickPlayers)
        {
            bl_PhotonNetwork.KickPlayer(bl_ScoreboardPopUpMenuBase.TargetPlayer);
        }
        else
        {
            // if the local player is not the master client, then he can request a votation to kick the player
