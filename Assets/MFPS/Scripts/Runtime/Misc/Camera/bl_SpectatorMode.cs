...
public class bl_SpectatorMode : bl_SpectatorModeBase
{
    ...
    public override void SetActiveSpectatorMode(bool active)
    {
        ...
        {
            SetActiveUI(false);
            if (bl_GameData.CoreSettings.onLeaveSpectatorMode == LeaveSpectatorModeAction.ReturnToLobby)
            {
                bl_UtilityHelper.LockCursor(false);
                bl_UIReferences.Instance.leaveRoomConfirmation.AskConfirmation(bl_GameTexts.LeaveMatchWarning.Localized("areusulega"), () =>
                {
                    bl_RoomMenu.Instance.LeaveRoom(RoomLeaveCause.UserAction, false);
        ...
        isActive = active;
        bl_EventHandler.Match.onSpectatorModeChanged?.Invoke(active);
    }
