...
public class bl_SimpleSceneLoader : bl_SceneLoaderBase
{
    ...
    public override void LoadMapScene(string mapSceneName)
    {
        if (!bl_MapSceneLoader.LoadMapScene(mapSceneName))
        {
            // If the map scene loader fails or not present, load the scene directly
