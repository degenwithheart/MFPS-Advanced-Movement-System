...
namespace MFPS.InputManager
{
    ...
    public class bl_GamePadPointerModule : PointerInputModule
    {
        ...
        void CheckImplementation()
        {
            ...
            {
                StandaloneInputModule sim = FindAnyObjectByType<StandaloneInputModule>();
                if (sim)
                {
        ...
        public override void Process()
        {
            ...
            Vector2 scrollDelta = new(bl_Input.HorizontalAxis, -bl_Input.VerticalAxis);
            pointer.scrollDelta = scrollDelta;
            if (!Mathf.Approximately(scrollDelta.sqrMagnitude, 0.0f))
            {
                var scrollHandler = ExecuteEvents.GetEventHandler<IScrollHandler>(pointer.pointerCurrentRaycast.gameObject);
                ExecuteEvents.ExecuteHierarchy(scrollHandler, pointer, ExecuteEvents.scrollHandler);
            }

            if (Input.GetKeyDown(bl_InputData.MappedInstance.submitButton))
            {
            ...
            else if (Input.GetKeyUp(bl_InputData.MappedInstance.submitButton))
            {
        ...
        public static bl_GamePadPointerModule Instance
        {
            get
            {
                if (m_instance == null) { m_instance = FindAnyObjectByType<bl_GamePadPointerModule>(); }
                return m_instance;
            }
        }
