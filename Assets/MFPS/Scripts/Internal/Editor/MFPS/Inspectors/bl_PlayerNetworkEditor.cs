...
public class bl_PlayerNetworkEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        m_SynchronizePositionProperty.isExpanded = MFPSEditorStyles.ContainerHeaderFoldout("Position Synchronization", m_SynchronizePositionProperty.isExpanded);
        if (m_SynchronizePositionProperty.isExpanded)
        {
            DrawSynchronizePositionData();
        }

        m_SynchronizeRotationProperty.isExpanded = MFPSEditorStyles.ContainerHeaderFoldout("Rotation Synchronization", m_SynchronizeRotationProperty.isExpanded);
        if (m_SynchronizeRotationProperty.isExpanded)
        {
        ...
        EditorGUILayout.BeginHorizontal();
        {
            ...
            if (GUILayout.Button("Ping Current TPWeapon", EditorStyles.toolbarButton))
            {
                ...
                else
                {
                    ...
                    {
                        var rw = m_Target.transform.GetComponentInChildren<bl_RemoteWeapons>(true);
                        if (rw != null)
                        {
                            Selection.activeObject = rw.transform;
                            EditorGUIUtility.PingObject(rw.transform);
                        }
                    }
                }
