...
public class bl_NamePlateEditor : Editor
{
    ...
    private void OnSceneGUI()
    {
        ...
        if (vector.z > 0)
        {
            ...
            vector.y += distanceDifference * (script.distanceModifier + 3.3f) + script.StylePresent.HealthBackOffset.y;

            int vertical = bl_GameData.CoreSettings.ShowTeamMateHealthBar ? 15 : 10;
            if (distance < script.playerNameDrawDistance)
            {
                GUI.Label(new Rect(vector.x - 5, (Screen.height - vector.y) - vertical, 10, 11), simulatedName, script.StylePresent.style);
                if (bl_GameData.CoreSettings.ShowTeamMateHealthBar)
                {
                    float mh = script.StylePresent.HealthBarWidth;
                    float h = script.StylePresent.HealthBarWidth * ((float)simulatedHealth / 100f);
                    GUI.color = script.StylePresent.HealthBackColor;
                    GUI.DrawTexture(new Rect(vector.x - (mh * 0.5f), (Screen.height - vector.y), mh, script.StylePresent.HealthBarThickness), script.StylePresent.HealthBarTexture);
                    GUI.color = script.StylePresent.HealthBarColor;
                    GUI.DrawTexture(new Rect(vector.x - (mh * 0.5f), (Screen.height - vector.y), h, script.StylePresent.HealthBarThickness), script.StylePresent.HealthBarTexture);
